...
class NodeType(Enum):
    """Enum to represent the type of a node in the execution graph."""
...
@dataclass
class ExecutionNode:
    """A node in the execution graph representing a Module and a data type."""

    module: Module
    data_type: Tuple[Type[Data], ...]
    node_type: NodeType

    def __hash__(self) -> int:
        """Hash function to allow the node to be used in sets and as dictionary keys."""
        return hash((self.module, self.data_type, self.node_type))


class ExecutionGraph:
    def __init__(self, root_module: "Module"):
        self.modules = root_module.get_modules()

        self.nodes = self._build_nodes()

        self.processes, self.process_mapping = self._build_processes()
        # (
        #     self.layers,
        #     self.layer_mapping,
    ...
    def _build_nodes(self) -> Set[ExecutionNode]:
        """Build the execution nodes from the modules."""
        nodes: Set[ExecutionNode] = set()

        for module in self.modules:
            for data_type in module._sourced_to_interface.keys():
                nodes.add(ExecutionNode(module, data_type, NodeType.SOURCE))
            for data_type in module._sinked_to_interface.keys():
                nodes.add(ExecutionNode(module, data_type, NodeType.SINK))
    ...
    def _build_processes(self):
        ...
        groups = {index: {node} for index, node in enumerate(self.nodes)}
        group_mapping = {node: index for index, node in enumerate(self.nodes)}

        for node in self.nodes:
            if node.node_type is not NodeType.SINK:
                # If the node is a source, it belongs to its own process
                continue
            connected = node.module._sinked_to_interface[
                ensure_tuple_format(node.data_type)
            ].connected
            for dependency_connect in connected:
                if not dependency_connect.sync:
                    continue
                node_group = group_mapping[node]
                dependency_node = ExecutionNode(
                    module=dependency_connect.module,
                    data_type=dependency_connect.data,
                    node_type=NodeType.SOURCE,
                )
                dependency_group = group_mapping[dependency_node]

                if node_group != dependency_group:
                    merged_group = groups[dependency_group].union(groups[node_group])
                    for merged_node in merged_group:
                        group_mapping[merged_node] = dependency_group
